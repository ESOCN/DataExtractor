...
--]] local DataExtractorDetails = {
    ...
    -- 选项。
    itemScanLimit = 400000, -- 扫描的物品ID数量上限。注意：最大值大约在30万左右。
    -- 数据。
    dataSkills = {}, -- 技能数据。
    dataCpSkills = {},
    dataSets = {}, -- 存放所有套装的引用。
    ...
    slashAbilities = '/scrapeabilities',
...
local function SaveData()
    ...
    DataExtractor.savedVariables.dataAbilities = DataExtractor.dataAbilities
...
function DataExtractor.OnAddOnLoaded(event, addonName)
    if addonName ~= DataExtractor.name then
        return
    end
    EVENT_MANAGER:UnregisterForEvent(DataExtractor.name, EVENT_ADD_ON_LOADED)

    DataExtractor.savedVariables = ZO_SavedVars:NewAccountWide("DataExtractorSavedVariables", 1, nil,
        DataExtractor.savedVariables)
    ...
    SLASH_COMMANDS[DataExtractor.slashAbilities] = DataExtractor.GetAllAbilities
